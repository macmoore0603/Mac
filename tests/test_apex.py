...
class TestInitialState:
    def test_fresh_account_starts_at_published_numbers(self, fresh_state):
        ...
        assert fresh_state.threshold == 47_500.0
        assert fresh_state.room == 2_500.0
    ...
    def test_resume_infers_peak_from_threshold(self):
        ...
        # Threshold 48,900 implies a peak of 48,900 + 2,500.
        assert state.peak_equity == pytest.approx(51_400.0)
...
class TestIntradayTrailing:
    ...
    def test_unrealised_profit_ratchets_the_threshold(self, fresh_state):
        ...
        assert fresh_state.threshold == 49_000.0
    ...
    def test_round_trip_to_breakeven_permanently_costs_room(self, fresh_state):
        ...
        assert fresh_state.threshold == 49_000.0       # but the threshold moved
        assert fresh_state.room == 1_000.0             # 60% of the buffer, gone
    ...
    def test_room_cost_is_predicted_before_the_fact(self, fresh_state):
        ...
        actual = 2_500.0 - fresh_state.room
    ...
    def test_profit_needed_to_lock_is_reported(self, fresh_state):
        assert fresh_state.profit_to_lock_threshold() == pytest.approx(2_600.0)
        fresh_state.mark(1_000.0)
        assert fresh_state.profit_to_lock_threshold() == pytest.approx(1_600.0)
        fresh_state.mark(2_600.0)
        assert fresh_state.profit_to_lock_threshold() == 0.0

    def test_breach_detected_at_the_threshold(self, fresh_state):
        fresh_state.mark(-2_499.0)
        assert not fresh_state.is_breached
        fresh_state.mark(-2_500.0)
...
class TestSizing:
    ...
    def test_dollar_limits_bind_before_the_contract_cap(self, fresh_state):
        ...
        With default limits the $250 per-trade cap is far more restrictive than
        the 100-micro account cap, so it is the per-trade cap that decides size.
        ...
        assert sizing.risk_budget == pytest.approx(250.0)
        assert sizing.quantity == 14  # floor(250 / 17.34)
...
class TestConsistency:
    @staticmethod
    def _trade_days(state, profits: list[float]) -> None:
    ...
    def test_reports_required_total_for_payout(self, fresh_state):
        # One outsized day is not a violation; it raises the bar for withdrawal.
        self._trade_days(fresh_state, [1_500.0, 500.0])
        status = consistency_status(fresh_state)
        assert status.best_day == pytest.approx(1_500.0)
        assert status.total_profit == pytest.approx(2_000.0)
        # Best day $1,500 at 30% requires $5,000 total profit.
        assert status.required_total == pytest.approx(5_000.0)
    ...
    def test_no_rule_configured_is_always_compliant(self):
        from dataclasses import replace

        profile = replace(APEX_50K_INTRADAY, consistency_pct=None)
        state = AccountState.fresh(profile)
        assert consistency_status(state).compliant
